...
const tanggal = function(a) {
    ...
    function buatO(newDate) {
        ...
        var seconds = format.substring(0, format.length - newDate.getSeconds().length) + newDate.getSeconds();
        var minutes = format.substring(0, format.length - newDate.getMinutes().length) + newDate.getMinutes();
        var hours = format.substring(0, format.length - newDate.getHours().length) + newDate.getHours();
        var showTimes = pad(hours, 2) + ':' + pad(minutes, 2) + ':' + pad(seconds, 2);

        return {
            full: dayKnow,
            day: newDate.getDay(),
            times: showTimes,
            timestamp: dayKnow + ' ' + showTimes
        };
    ...
    var returnData = {
        ...
        times: buatO(newDate).times,
        timestamp: buatO(newDate).timestamp,
        cek1: buatC(newDate),
