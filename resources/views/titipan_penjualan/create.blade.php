...
@section("form-create")
    <form action="{{ route("titipan_penjualan.store") }}" method="POST">
        @csrf @method("POST")
        ...
        <div class="row">
            <div class="col">
                <div class="d-flex flex-column mb-7 fv-row">
                    <label class="d-flex align-items-center fs-6 fw-bold form-label mb-2">
                        <span class="required">{{ ucwords("kas bank") }}</span>
                    </label>
                    <select name="bank" class="form-control">
                        <option disabled selected value> -- select an option -- </option>
                        @foreach ($kas_banks as $kas_bank)
                            <option value="{{ $kas_bank->kode }}">{{ $kas_bank->kode }} | {{ $kas_bank->keterangan }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
